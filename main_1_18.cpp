...
static const int MAXS = 1024*1024*1024;  // 最大可以存储 1024M 的数据
...
double weight_param[DIMEN_DATA];      // 权重系数
...
void* store_train_data_block(void* __args)
{
    ...
    int buf_lens = arg->buf_lens;
    double dataV;
    int row = 0;
    int clo = 0;
    int char_index = 0;
    for (; char_index < buf_lens; ++row)
    {
        clo &= 0x00;
        for (; clo < arg->feature_nums; ++clo)
        {
            ...
            {
                dataV = train_buf[char_index + 1] + 0.1*train_buf[char_index + 3] + \
                    0.01*train_buf[char_index + 4] + 0.001*train_buf[char_index + 5] - 53.328;
                dataV = -1*dataV;
                char_index += 7;
            }
            else
            {
                dataV = train_buf[char_index] + 0.1*train_buf[char_index + 2] + \
                    0.01*train_buf[char_index + 3] + 0.001*train_buf[char_index + 4] - 53.328;
                    char_index += 6;
            }
    ...
    *(arg->data_lines) = row;   // 更新真实的 train_rowN 的值
    return NULL;
}
...
void* store_test_data_block(void* __args)
{
    ...
    features_labels* store_box = arg->store_box;
    int buf_lens = arg->buf_lens;
    int feature_nums = arg->feature_nums;
    double dataV;
    int row = 0;
    int clo = 0;
    int char_index = 0;
    ...
    {
        clo &= 0x00;
        for (; clo < feature_nums; ++clo)
        {
            if(test_buf[char_index] == '-')
            {
                dataV = test_buf[char_index+1] + 0.1*test_buf[char_index+3]+ \
                    0.01*test_buf[char_index+4] + 0.001*test_buf[char_index+5] - 53.328;
                dataV = -1*dataV;
                char_index += 7;
            }
            else
            {
                dataV = test_buf[char_index] + 0.1*test_buf[char_index+2] + \
                    0.01*test_buf[char_index+3] + 0.001*test_buf[char_index +4] - 53.328;
                char_index += 6;
            }
            store_box[row].features[clo] = dataV;
        }
    }
    *(arg->data_lines) = row;           // 更新真实的 test_rowN  的值
    return NULL;
}
...
// 使用两个线程,一个线程读取训练数据,另一个线程读取测试数据
    s_args[0].store_box = TrainData_Set;
    s_args[0].buf_lens = train_len;
    s_args[0].feature_nums = cloN;
    s_args[0].data_lines = &train_rowN;
    s_args[1].store_box = TestData_Set;
    s_args[1].buf_lens = test_len;
    s_args[1].feature_nums = cloN;
    s_args[1].data_lines = &test_rowN;

    add_task_2_tpool(thread_pool, &store_train_data_block, (void*)(&s_args[0]));

    for(i = 0; i < DIMEN_DATA; ++i)
        weight_param[i] = 0.0;
...
#ifdef _TEST_
    end_t = clock();
    std::cout << "store train file using time " << end_t - start_t << '\n';
    start_t = clock();
#endif

    add_task_2_tpool(thread_pool, &store_test_data_block, (void*)(&s_args[1]));
...
#ifdef _TEST_
    end_t = clock();
    std::cout << "train and load test file using time " << end_t - start_t << '\n';
    start_t = clock();
#endif
...
#ifdef _TEST_
    end_t = clock();
    std::cout << "predict time using " << end_t - start_t << '\n';
#endif
