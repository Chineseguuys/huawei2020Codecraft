		...
		const int& __head_id)
{
	...
	{
		if(child->state == HAS_VISITED)
			continue;

		if(child->state == IS_VISITING && child->ID == __head_id)
		{
		...
		else if (child->state == NOT_VISITED && __node_queue.size() < MAX_CIRCLE_LEN)
			circuit(child, __all_paths, __node_queue, __head_id);
	}
			...
			unordered_map<unsigned, graph_node*>& __fast_id_nodePtr_refl)
{
	...
	for (iter = __id_nodePtr_refl.begin(); iter != __id_nodePtr_refl.end(); ++iter)
	{
		...
		circuit(node, all_paths, index_store, node->ID);
