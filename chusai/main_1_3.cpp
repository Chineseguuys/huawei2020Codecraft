...
void create_graph(char* __data_buf, map<unsigned, graph_node*>& __id_nodePtr_refl, const int& __buf_len)
{
	int index  = 0;
	int from, to;
...
void circuit(graph_node* __node, map<unsigned int, graph_node*>& __id_nodePtr_refl,
		...
		std::vector<int> __node_queue, const int& __head_id)
{
		...
		//如果查找的过程中回到了起点的位置,那么就说明找到了一个环
	{
		if(__node_queue.size() >= 3)	// 需要保证环的长度大于等于 3 
		{
			...
			__all_paths.push_back(temp);	// 这个地方使用了 One_Cycle(const One_Cycle&) 构造函数
		}
