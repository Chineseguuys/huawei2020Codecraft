...
/**
	查看在每一次的迭代的过程中,遇到过多少个局部环
	局部环的个数是爆炸性的,大量的局部环造成了时间上的浪费
 */
#include <iostream>
#include <algorithm>
#include <map>
...
void circuit(graph_node* __node, unordered_map<unsigned int, graph_node*>& __id_nodePtr_refl,
		...
		const int& __head_id,
		unsigned int& __local_cycle_count)
{
	...
	if(__node->state == NOT_VISITED)
	{
		...
		for(graph_node* child : next_node)
		{
			...
			circuit(child, __id_nodePtr_refl, __all_paths, __node_queue, __head_id, __local_cycle_count);
			...
			unordered_map<unsigned, graph_node*>& __fast_id_nodePtr_refl)
{
	...
	unsigned int local_cycle_count;
	std::vector<unsigned int> local_cycle_counts;
	...
	for (iter = __id_nodePtr_refl.begin(); iter != __id_nodePtr_refl.end(); ++iter)
	{
		...
		local_cycle_count = 0;
		circuit(node, __fast_id_nodePtr_refl, all_paths, index_store, node->ID, local_cycle_count);
		node->state = HAS_VISITED;
		local_cycle_counts.push_back(local_cycle_count);
	}
	...
	std::cout << "local cycle nums " << '\n';

	for (unsigned int& item : local_cycle_counts)
		std::cout << item << " ";
