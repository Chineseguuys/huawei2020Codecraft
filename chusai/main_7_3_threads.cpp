...
/**
	这个版本将进行将排序考虑到里面去 
	尝试将 map<unsigned, graph_node*> 添加到求强连通分量函数当中,方便我们进行排序的过程
 */
...
void second_DFS(graph_node* __node, 
		...
		unordered_map<unsigned, graph_node*>& __fast_a_SCC,
		map<unsigned, graph_node*>& __a_SCC)
{
	has_visited.insert({__node->ID, __node});

	__a_SCC.insert({__node->ID, __node});
	__fast_a_SCC.insert({__node->ID, __node});

	for (graph_node* child_node : __node->inverse_childs)
	{
		if(has_visited.find(child_node->ID) != has_visited.end())
			continue;
		second_DFS(child_node, has_visited, __fast_a_SCC, __a_SCC);
	}
}
...
void stronge_conn_comp(unordered_map<unsigned, graph_node*>& __id_nodePtr_refl,
		std::vector<unordered_map<unsigned, graph_node*> >& __fast_all_SCCs,
		std::vector<map<unsigned, graph_node*> >& __all_SCCs)
{
	...
	unordered_map<unsigned, graph_node*> fast_a_SCC;
	map<unsigned, graph_node*> a_SCC;
	...
	while(top != -1)
	{
		...
		second_DFS(node, has_visited, fast_a_SCC, a_SCC);
		if (a_SCC.size() >= 3)	
		{
			// 因为我们要查找的环,都是长度要大于 3 的环,所以如果一个强连通分量的元素个数小于3的话,就可以直接剔除了
			__all_SCCs.push_back(std::move(a_SCC));
			__fast_all_SCCs.push_back(std::move(fast_a_SCC));
		}
	}
...
void* push_SCC_into_threadpool(void* __args)
{
	...
	map<unsigned, graph_node*>* a_SCC = args->a_SCC;

	graph_node* node = nullptr;
	std::vector<int> node_stack;

	map<unsigned, graph_node*>::iterator iter;
...
int main(int argc, char** argv)
{
	...
	const char result_file[] = "./result_7_3.txt";
#endif
	...
	std::vector<unordered_map<unsigned, graph_node*> > fast_all_SCCs;
	std::vector<map<unsigned, graph_node*> > all_SCCs;
	...
	stronge_conn_comp(fast_id_nodePtr_refl, fast_all_SCCs, all_SCCs);	// 找全部的强连通分量
...
#ifdef _TEST_
	std::cout << "have SCC " << fast_all_SCCs.size() << '\n';
#endif

	thread_func_args* t_func_args = new thread_func_args[fast_all_SCCs.size()];
	int index = 0;
	for (map<unsigned, graph_node*>& scc : all_SCCs)
	{
	...
	// 保存文件到指定的文件当中,注意文件的权限
	fd = open(result_file, O_RDWR | O_CREAT, 0666);
	int path_len;
	int num_str_len;
	char buffer[32];
