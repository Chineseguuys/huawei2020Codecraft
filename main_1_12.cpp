...
struct fileAnysis_thread_func_arg
{
    char* store_buf;
    int* split;
    int* count;
...
void* get_clomn_nums(void* __args)
{
    fileAnysis_thread_func_arg* arg = (fileAnysis_thread_func_arg*)(__args);
    *arg->count = 0;
    for (int i = 0; arg->store_buf[i] != '\n'; ++i)
        if(arg->store_buf[i] == ',')
            ++(*arg->count);
    return NULL;
}
...
void* get_line_nums(void* __args)
{
    fileAnysis_thread_func_arg* arg = (fileAnysis_thread_func_arg*)(__args);
    *arg->count = 0;
...
{
    fileAnysis_thread_func_arg args[3];
    FILE* fp = fopen(trainFile, "rb");
    train_len = fread(train_buf, 1, MAXS, fp);
    args[0].store_buf = args[1].store_buf = train_buf;
    args[0].len = args[1].len = train_len;
    args[0].count = &train_rowN;
    args[0].split = Trainline_split;
    args[1].count = &cloN;
    add_task_2_tpool(thread_pool, &get_line_nums, (void*)(&args[0]));
    add_task_2_tpool(thread_pool, &get_clomn_nums, (void*)(&args[1]));
    fclose(fp);

    fp = fopen(testFile, "rb");
    test_len = fread(test_buf, 1, MAXS, fp);
    args[2].store_buf = test_buf;
    args[2].len = test_len;
    args[2].count = &test_rowN;
    args[2].split = Testline_split;
    add_task_2_tpool(thread_pool, &get_line_nums, (void*)(&args[2]));
    fclose(fp);
    while(true)
    {
        usleep(1000);
